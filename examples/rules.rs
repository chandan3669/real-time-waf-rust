...
mod tests {
    ...
    fn test_unique_ids() {
        ...
        for rule in rules {
            if !ids.insert(rule.id.clone()) {
                panic!("Duplicate rule ID found: {}", rule.id);
            }
        }
    }
    ...
    fn test_regex_validity() {
        // Just calling load_default_rules() tests regex validity due to expect()
        let _rules = load_default_rules();
    }
    
    #[test]
    fn test_sqli_detection() {
        let rules = load_default_rules();
        let sqli_rule = rules.iter().find(|r| r.id == "SQL-001").unwrap();
        assert!(sqli_rule.matches("user input UNION SELECT * FROM users"));
        assert!(sqli_rule.matches("1' union select 1,2,3--"));
        assert!(!sqli_rule.matches("SELECT * FROM products WHERE id=1")); // Parameterized query lookalike
    }

    #[test]
    fn test_xss_detection() {
         let rules = load_default_rules();
        let xss_rule = rules.iter().find(|r| r.id == "XSS-001").unwrap();
        assert!(xss_rule.matches("<script>alert(1)</script>"));
        assert!(xss_rule.matches("Value <SCRIPT>evil()</SCRIPT>"));
        assert!(!xss_rule.matches("This is a script about Java."));
    }
}
